...
So the first thing that we are going to do is move code around to multiple files.
...
In this section of tutorial, we introduced the sending an `Event` over a channel using a separate thread.
This is essentially the setup used in the official [template](https://github.com/tui-rs-revival/rust-tui-template/tree/master/src).

Using channels can make it easy to separate concerns and isolate different parts of your code.
In the next section, we'll introduce `tokio` and `async` to do the same thing.
