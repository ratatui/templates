...
The `terminal.draw(|f| { ui(app, f); })` call is the only line in the code above
that uses `ratatui` functionality.
...
[`Frame`](https://docs.rs/ratatui/latest/ratatui/terminal/struct.Frame.html) and expects
